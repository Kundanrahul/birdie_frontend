...
const MainProfile = ({ user }) => {
  ...
  useEffect(() => {
    fetch(`https://birdie-backend-ux74.onrender.com/userpost?email=${user?.email}`)
      .then(res => res.json())
      .then(data => {
        setPosts(data);
      })
  ...
  const handleUploadCoverImage = e => {
    ...
    axios.post("https://api.imgbb.com/1/upload?key=fda45f6cd272b13a34ca62edb992bf02", formData)
      .then(res => {
        ...
        if (url) {
          fetch(`https://birdie-backend-ux74.onrender.com/userUpdates/${user?.email}`, {
            method: "PATCH",
            headers: {
